...
class CoffeeCup extends Component {
  state = {ready: false, selectedTab: 'redTab', notifCount: 0, presses: 0};
  ...
  async _loadInitialState() {
    try {
      ...
      this.setState({ready: true, login: this.oauthClient.hasTokens()});
    } catch (err) {
  ...
  renderLogin() {
    return (
      <LoginScreen oauthClient={this.oauthClient} onAccessTokenFetched={this.onAccessTokenFetched} />
    );
  }
    ...
    return (
      <TabBarIOS>
        <TabBarIOS.Item
          ...
          <HomeScreen oauthClient={this.oauthClient} />
        </TabBarIOS.Item>
        <TabBarIOS.Item
          systemIcon="history"
